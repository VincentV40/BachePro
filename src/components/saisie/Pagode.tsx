...
import type { PagodeParams, OeilletConfig, OptionsPatronage } from "@/engine/types";
import { genererPanneaux } from "@/engine/typologies/pagode";
...
export default function Pagode({ projetId }: Props) {
  ...
  const params = projet?.params as PagodeParams | undefined;
  ...
  const setParam = useCallback(
    <K extends keyof PagodeParams>(key: K, value: PagodeParams[K]) => {
      if (!params) return;
        ...
        <Card className="shadow-sm">
          ...
          <CardContent>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <DimensionField label="Hauteur faitage" value={params.hauteur_faitage_mm} onChange={(v) => setParam("hauteur_faitage_mm", v)} />
              <DimensionField label="Hauteur bords" value={params.hauteur_bords_mm} onChange={(v) => setParam("hauteur_bords_mm", v)} />
