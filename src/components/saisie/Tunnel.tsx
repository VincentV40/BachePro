...
import type { TunnelParams, OeilletConfig, OptionsPatronage } from "@/engine/types";
import { genererPanneaux } from "@/engine/typologies/tunnel";
...
export default function Tunnel({ projetId }: Props) {
  ...
  const params = projet?.params as TunnelParams | undefined;
    ...
    <div className="p-4 md:p-6 grid gap-6 lg:grid-cols-[1fr_320px]">
      <div className="space-y-5">
        ...
        <Card className="shadow-sm">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm flex items-center gap-2"><Ruler className="w-4 h-4 text-primary" /> Dimensions</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid gap-4 sm:grid-cols-2">
              <DimensionField label="Largeur (diametre)" value={params.largeur_mm} onChange={(v) => setParam("largeur_mm", v)} />
              <DimensionField label="Profondeur (longueur)" value={params.profondeur_mm} onChange={(v) => setParam("profondeur_mm", v)} />
              <DimensionField label="Hauteur au sommet" value={params.hauteur_mm} onChange={(v) => setParam("hauteur_mm", v)} />
              <DimensionField label="Nombre de facettes" value={params.nb_facettes} onChange={(v) => setParam("nb_facettes", v)} suffix="" />
            </div>
          </CardContent>
        </Card>
      ...
      <div className="lg:sticky lg:top-[65px] lg:self-start space-y-4">
        <Card className="shadow-md border-primary/10 overflow-hidden">
          ...
          {!resultat && (<CardContent className="pt-4"><p className="text-sm text-muted-foreground">Renseignez les dimensions et le tissu pour voir le resultat.</p></CardContent>)}
        </Card>
      </div>
    </div>
  );
