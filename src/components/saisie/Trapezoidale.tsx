...
export default function Trapezoidale({ projetId }: Props) {
  ...
  const params = projet?.params as TrapezoidaleParams | undefined;
  ...
  const setParam = useCallback(
    <K extends keyof TrapezoidaleParams>(key: K, value: TrapezoidaleParams[K]) => {
      if (!params) return;
        ...
        <Card className="shadow-sm">
          ...
          <CardContent>
            <div className="grid gap-4 sm:grid-cols-2">
              <DimensionField label="Largeur avant" value={params.largeur_avant_mm} onChange={(v) => setParam("largeur_avant_mm", v)} />
              <DimensionField label="Largeur arriere" value={params.largeur_arriere_mm} onChange={(v) => setParam("largeur_arriere_mm", v)} />
